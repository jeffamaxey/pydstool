...
class ADMC_ODEsystem(ODEsystem):
    ...
    def __init__(self, kw):
        ...
        self._model_file = f"{self.name}.m"
        self._ic_file = f"{self.name}_ics.m"
        self._param_file = f"{self.name}_params.m"
        self._set_file = "set.m"
        self._get_file = "get.m"
        self._vfield_file = "vfield.m"
        self._events_file = "events.m"

        self._vf_filename_ext = f"_{self._model_file[:-2]}"
...
#       eventTol = []
    ...
    def _prepareEventsFileContents(self):
        ...
        for x in range(evcount):
            ...
            ev = self.eventstruct.events[evname[x]]
            evsig = f"function {ev._LLreturnstr}{ev.name}{ev._LLargstr}" + "\n"
            assert ev._LLfuncstr.index(';') > 1, ("End your event function with a ';'")

            fbody = f"%BEGIN func {ev.name}" + "\n" + alldefines
            fbody += ev._LLfuncstr
    ...
    def _prepareSetFileContents(self):
        ...
        commentstr = (
            f"% Set method for model {self.name}"
            + "\n% Generated by PyDSTool for ADMC++ target\n\n"
        )
        ...
        return topstr + commentstr + bodystr + parentfieldstr
    ...
    def _prepareGetFileContents(self):
        ...
        commentstr = (
            f"% Get method for model {self.name}"
            + "\n% Generated by PyDSTool for ADMC++ target\n\n"
        )
    ...
    def _prepareVfieldContents(self, vfdefines):
        ...
        commentstr = (
            f"% Vector field definition for model {self.name}"
            + "\n% Generated by PyDSTool for ADMC++ target\n\n"
        )

        bodystr = vfdefines

        return topstr + commentstr + bodystr + self.funcspec.spec[0]
    ...
    def _prepareModelContents(self):
        allfilestr = ""
        topstr = f"function a = {self.name}" + "(varargin)\n"
        commentstr = (
            f"% Vf object definition for model {self.name}"
            + "\n% Generated by PyDSTool for ADMC++ target\n\n"
        )
        ...
        return topstr + commentstr + bodystr

    def _prepareICContents(self):
        allfilestr = ""
        topstr = f"function ics_ = {self.name}" + "_ics()\n"
        commentstr = (
            f"% Initial conditions for model {self.name}"
            + "\n% Generated by PyDSTool for ADMC++ target\n\n"
        )

        bodystr = "ics_ = [ ...\n"
        if self.initialconditions:
            icnames = sorted(self.initialconditions.keys())
            for i in range(len(icnames)-1):
                ...
                bodystr += f"{val}, ... % {icnames[i]}" + "\n"

            if isnan(self.initialconditions[icnames[-1]]):
                val = str(0.0)
            else:
                val = self.initialconditions[icnames[-1]]

            bodystr += f"{val} % {icnames[-1]}" + " ...\n"
    ...
    def _prepareParamContents(self):
        allfilestr = ""
        topstr = f"function pars__ = {self.name}" + "_params()\n"
        commentstr = (
            f"% Parameters for model {self.name}"
            + "\n% Generated by PyDSTool for ADMC++ target\n\n"
        )

        bodystr = "pars_ = [ ...\n"
        if self.pars:
            pnames = sorted(self.pars.keys())
            for i in range(len(pnames)-1):
                bodystr += f"{str(self.pars[pnames[i]])}, ... % {pnames[i]}" + "\n"

            bodystr += f"{str(self.pars[pnames[-1]])} % {pnames[-1]}" + " ...\n"
    ...
    def _prepareEventFuncStrings(self, vfdefines):
        ...
        numevs = 0 if self._eventNames == [] else len(self._eventNames)
        evfullfn = ""
        for ev in self._eventNames:
            ev = self.eventstruct.events[evname]
            assert isinstance(ev, MatlabEvent), ("ADMC can only accept matlab events")
            evsig = f"function y_ = {ev.name}" + "(vf_, t_, x_, p_)\n"
...
#        allaux =
    ...
    def makeLibSource(self):
        ...
        try:
            ...
            print(f"Error opening file {self._model_file} for writing")
            raise IOError(e)
        ...
        if len(self._eventNames) > 0:
            ...
            try:
                ...
                print(f"Error opening file {self._events_file} for writing")
                raise IOError(e)
        ...
        try:
            ...
            print(f"Error opening file {self._ic_file} for writing")
            raise IOError(e)
        ...
        try:
            ...
            print(f"Error opening file {self._param_file} for writing")
            raise IOError(e)
        ...
        try:
            with open(getfile, 'w') as file:
                file.write(allfilestr)
        except IOError as e:
            print(f"Error opening file {self._get_file} for writing")
            raise IOError(e)
        ...
        try:
            with open(setfile, 'w') as file:
                file.write(allfilestr)
        except IOError as e:
            print(f"Error opening file {self._set_file} for writing")
            raise IOError(e)
...
#        allfilestr = self._prepareVfieldContents(vfdefines)
        ...
        try:
            with open(vffile, 'w') as file:
                file.write(allfilestr)
        except IOError as e:
            print(f"Error opening file {self._vfield_file} for writing")
            raise IOError(e)
